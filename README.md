...
1. Runnable Scripts and Commands:
   ...
   - CosineLR_EfficiencyNetModel AndFilteredPredictions(PENKRAT).ipynb
...
2. Contributions:
   ...
   -   Lindsay Penkrat was responsible for reading and contributing to related literature on our subject as well as reading through documentation and DeepLabCut code to breakdown and explain how the package works. She communicated that infomraiton in the final presentation as well as the Related Works and Methods section of our final report. She also setup the Github environment. She was further responsible for the experiments on filtering out noisy data from predictions and testing the efficiencynet-b5 model for the cosine learning rate against the resnet-50 model with an exponential learning rate.
   -  Ashkan Moghaddassi was responsible for the formulation of the project objectives and the implementation of DeepLabCut on the GPU machine. He wrote Python scripts for color segmentation cropping of the video frames which attempted to improve runtime efficiency and labeling accuracy. Files and results can be found in the folder entitled "color_segmentation." Note: color_segmentation.py could not be implemented readily into the extract_frames function, so we used dynamic cropping instead, which did not require much modification of the original code. 
